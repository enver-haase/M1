...
extern unsigned char *prgrom;

int adsp2100_fullcount;
...
{
	// hack for 1st-generation DCS pinball games
	if (addr == 0x3000)
	{
		return adsp_read32(0xffffffff);
	}

	addr <<= 2;
	return *(UINT32 *)&prgrom[ADSP2100_PGM_OFFSET + addr];
...
//	printf("WWORD_PGM: %x @ %x\n", data, addr);
	ADSP2100_WRPGM(&prgrom[ADSP2100_PGM_OFFSET + addr], data);
}
...
//	printf("generate IRQ: which %d indx %d OK (imask %x)\n", which, indx, adsp2100.imask);
...
void adsp2104_set_irq_line(int irqline, int state)
{
...
int adsp2100_getcycles(void)
{
	return (adsp2100_fullcount - adsp2100_icount);
}

void adsp2100_yield(void)
{
	adsp2100_fullcount = adsp2100_getcycles();
...
int adsp2100_execute(int cycles)
{
	...
	adsp2100_fullcount = adsp2100_icount;
...
//		if (adsp2100.pc == 0xa1) golog = 1;
//		if (golog) printf("[%04x]: %08x AX0 = %x AR = %x AF = %x SI = %x\n", adsp2100.pc, op, adsp2100.core.ax0.u, adsp2100.core.ar.u, adsp2100.core.af.u, adsp2100.core.si.u);
